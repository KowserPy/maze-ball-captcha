...
import type { GameState, MazeGameProps } from '../types/maze';

const MazeGameContainer: React.FC<MazeGameProps> = ({ onComplete }) => {
  ...
  const handleGameComplete = async (solutionPath: string) => {
    ...
    try {
      const response = await fetch('https://gsvikasictrxsywbkrlp.supabase.co/functions/v1/validate-maze-completion', {
        method: 'POST',
        headers: {
      ...
      >
        {!gameStarted ? (
          ...
          <>
            ...
            <MazeControls
              gameState={gameState}
              onGenerateNewMaze={generateNewMazeHandler}
            />
          </>
        )}
      </motion.div>
